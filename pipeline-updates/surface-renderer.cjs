...
// sit there until someone noticed and manually killed it. 10 minutes is
// generous for a single ffmpeg invocation on one segment/act (real
// full-length renders in this codebase run in seconds to a couple of
// minutes per call), while still eventually surfacing a hang as a real,
...
function run(cmd, label) {
  ...
  try {
    ...
    if (err.signal || err.killed) {
      throw new Error(
        `FFmpeg timed out after ${FFMPEG_TIMEOUT_MS / 1000}s [${label}] and was killed. ` +
        `This usually means a hung or corrupted input file rather than a normal encode.`
      );
    }
...
function getAudioDuration(filePath) {
  ...
  try {
    out = execSync(
      ...
      { maxBuffer: 8 * 1024 * 1024, timeout: FFPROBE_TIMEOUT_MS }
    ).toString().trim();
  } catch (err) {
    if (err.signal || err.killed) {
      throw new Error(`ffprobe timed out after ${FFPROBE_TIMEOUT_MS / 1000}s reading duration of "${filePath}"`);
    }
    throw err;
  }
...
function getVideoDimensions(filePath) {
  ...
  try {
    ...
    if (err.signal || err.killed) {
      throw new Error(`ffprobe timed out after ${FFPROBE_TIMEOUT_MS / 1000}s reading dimensions of "${filePath}"`);
    }
    throw err;
  }
