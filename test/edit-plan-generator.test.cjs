...
test('repair policy version 4 changes scope while generation fingerprint stays stable', () => {
  assert.equal(_repairPolicyVersion, 4);
  assert.notEqual(_repairAttemptScopeFingerprint({ generationFingerprint: 'same', repairPolicyVersion: 3 }), _repairAttemptScopeFingerprint({ generationFingerprint: 'same', repairPolicyVersion: 4 }));
});
